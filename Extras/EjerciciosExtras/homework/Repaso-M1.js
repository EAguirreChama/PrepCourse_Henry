...
var countProps = function(obj) {
    ...
    for (let prop in obj) {
        // Para ir aumentando el contador
        contador ++
        if (typeof obj[prop] === "object") {
            // Para saber si es un array Array.isArray(loquesequieresabersiesarray)
...
LinkedList.prototype.changeNotNumbers = function(){
    // Tu código aca:
    // Un contador para ir contando los cambios
    let contador = 0;
    let current = this.head
    //Recorrer mientras exista current
    while (current) {
        // Condicional
        // isNaN => true (no es un numero) fale (es un numero)
        let number = Number(current.value) // Para convertir a numero utilizo Number
        if (isNaN(number)){ 
            contador ++ // Para que aumente el contador
